...
def _map_requested_channel_to_boundary_reason(requested_channel: str | None):
...
def classify_noise_boundary_reason(reason: str):
    requested_channel = _extract_requested_channel(reason)
    if "Unsupported density-noise channel" in reason:
        first_condition = _map_requested_channel_to_boundary_reason(requested_channel)
        if first_condition in DEFERRED_FAMILY_REASONS:
        ...
        return {
            ...
            "noise_boundary_class": BOUNDARY_CLASS_MODEL_FAMILY,
    ...
    if "after_gate_index must be non-negative" in reason:
        return {
            ...
            "noise_boundary_class": BOUNDARY_CLASS_SCHEDULE_ELEMENT,
    ...
    if "target_qbit out of range" in reason:
        return {
            ...
            "noise_boundary_class": BOUNDARY_CLASS_CONFIGURATION,
            "failure_stage": "cxx_noise_spec_validation",
            "requested_noise_channel": None,
            **build_unsupported_case_classification(
    ...
    return {
        ...
        "noise_boundary_class": "workflow_execution",
