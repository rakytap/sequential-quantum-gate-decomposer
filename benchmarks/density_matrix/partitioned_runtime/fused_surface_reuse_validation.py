...
SUITE_NAME = "phase3_partitioned_runtime_fused_surface_reuse"
ARTIFACT_FILENAME = "surface_reuse_bundle.json"
DEFAULT_OUTPUT_DIR = (
    REPO_ROOT
    / "benchmarks"
    / "density_matrix"
...
def build_artifact_bundle(cases: list[dict]) -> dict:
    ...
    actual_fused_cases = sum(case["summary"]["actual_fused_execution"] for case in cases)
    bundle = {
        ...
        "summary": {
            ...
            "actual_fused_cases": actual_fused_cases,
        },
        "cases": cases,
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.quiet:
        for case in cases:
            print(
                "{case_name}: runtime_path={runtime_path}, actual_fused={actual_fused_execution}".format(
                    case_name=case["case_name"],
                    runtime_path=case["runtime_path"],
                    actual_fused_execution=case["summary"]["actual_fused_execution"],
                )
            )
