    ...
    case.update(
        {
            ...
            "continuity_descriptor_pass": (
                payload["schema_version"] == PHASE3_DESCRIPTOR_SCHEMA_VERSION
...
def build_artifact_bundle(cases: list[dict]) -> dict:
    ...
    descriptor_passes = sum(case["continuity_descriptor_pass"] for case in cases)
    bundle = {
        ...
        "summary": {
            ...
            "continuity_descriptor_passes": descriptor_passes,
    ...
    if missing:
        raise ValueError(
            "Continuity descriptor bundle missing required fields: {}".format(
                ", ".join(missing)
            )
        )
    return bundle
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Directory to write the continuity descriptor bundle into.",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
