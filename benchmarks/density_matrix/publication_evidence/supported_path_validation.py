...
SUITE_NAME = "phase3_task8_story5_supported_path_scope"
ARTIFACT_FILENAME = "supported_path_scope_bundle.json"
DEFAULT_OUTPUT_DIR = task8_story_output_dir("story5_supported_path_scope")
STORY4_PATH = task8_story_output_dir("story4_evidence_closure") / STORY4_ARTIFACT_FILENAME
ARTIFACT_FIELDS = (
    "suite_name",
    "status",
...
def build_surface_inventory():
    ...
    for surface_id in PUBLICATION_SURFACES:
        ...
        entry["task6_counts_present"] = {
            "task6_count": "task6_count" not in missing_ids,
            "task6_boundary_count": "task6_boundary_count" not in missing_ids,
        }
        entry["task7_counts_present"] = {
            "task7_count": "task7_count" not in missing_ids,
            "task7_review_cases": "task7_review_cases" not in missing_ids,
        }
...
def build_artifact_bundle():
    ...
    all_task6_count_surfaces_honest = all(
        all(entry["task6_counts_present"].values()) for entry in surface_inventory
    )
    all_task7_count_surfaces_honest = all(
        all(entry["task7_counts_present"].values()) for entry in surface_inventory
    )
    ...
    supported_path_scope_completed = all(
        [
            ...
            all_task6_count_surfaces_honest,
            all_task7_count_surfaces_honest,
    ...
    artifact = {
        ...
        "summary": {
            ...
            "all_task6_count_surfaces_honest": all_task6_count_surfaces_honest,
            "all_task7_count_surfaces_honest": all_task7_count_surfaces_honest,
...
def validate_artifact_bundle(artifact):
    ...
    expected_flags = {
        ...
        "all_task6_count_surfaces_honest": all(
            all(entry["task6_counts_present"].values())
            for entry in artifact["surface_inventory"]
        ),
        "all_task7_count_surfaces_honest": all(
            all(entry["task7_counts_present"].values())
            for entry in artifact["surface_inventory"]
        ),
        "all_diagnosis_limitations_present": all(
            entry["diagnosis_closure_present"] and entry["bottleneck_wording_present"]
    ...
    expected_completed = all(
        [
            artifact["story4_evidence_closure"]["status"] == "pass",
            ...
            artifact["summary"]["all_task6_count_surfaces_honest"],
            artifact["summary"]["all_task7_count_surfaces_honest"],
