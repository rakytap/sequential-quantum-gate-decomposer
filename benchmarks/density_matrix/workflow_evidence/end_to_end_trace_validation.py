...
def get_required_end_to_end_qubits(workflow_contract):
    return tuple(workflow_contract["thresholds"]["required_end_to_end_qubits"])


def get_required_trace_case_name(workflow_contract):
    return workflow_contract["input_contract"]["execution_modes"][
...
def build_mandatory_end_to_end_case_names(workflow_contract):
    return tuple(
        ...
        for qbit_num in get_required_end_to_end_qubits(workflow_contract)
    )


def build_requirement_metadata(workflow_contract):
    mandatory_end_to_end_qubits = get_required_end_to_end_qubits(workflow_contract)
    mandatory_case_names = build_mandatory_end_to_end_case_names(workflow_contract)
    required_trace_case_name = get_required_trace_case_name(workflow_contract)
    return {
...
def validate_end_to_end_case_payload(case):
    ...
    if missing_fields:
        raise ValueError(
            "End-to-end trace case is missing required fields: {}".format(
                ", ".join(missing_fields)
            )
        )
...
def validate_trace_artifact(trace_artifact):
    ...
    if missing_fields:
        raise ValueError(
            "End-to-end trace artifact is missing required fields: {}".format(
                ", ".join(missing_fields)
            )
        )
...
def _enrich_end_to_end_case(case, workflow_contract):
    ...
    case["workflow_evidence_role"] = "required_end_to_end"
...
def _enrich_trace_artifact(trace_artifact, workflow_contract):
    ...
    trace_artifact["workflow_id"] = workflow_contract["workflow_id"]
    trace_artifact["contract_version"] = workflow_contract["contract_version"]
    trace_artifact["workflow_evidence_role"] = "required_trace"
...
def _extract_mandatory_cases(workflow_bundle, workflow_contract):
    selected_cases = []
    for case_name in build_mandatory_end_to_end_case_names(workflow_contract):
        matching_cases = [
            case for case in workflow_bundle["cases"] if case["case_name"] == case_name
        ]
        if matching_cases:
            selected_cases.append(
                _enrich_end_to_end_case(matching_cases[0], workflow_contract)
            )
...
def build_artifact_bundle(workflow_contract, workflow_bundle, trace_artifact):
    ...
    mandatory_end_to_end_qubits = get_required_end_to_end_qubits(workflow_contract)
    required_trace_case_name = get_required_trace_case_name(workflow_contract)
    cases = _extract_mandatory_cases(workflow_bundle, workflow_contract)
    for case in cases:
        validate_end_to_end_case_payload(case)

    trace_artifact = _enrich_trace_artifact(trace_artifact, workflow_contract)
    ...
    threshold_metadata = build_threshold_metadata(workflow_contract, workflow_bundle)
    ...
    trace_matches_contract = bool(
        trace_artifact["workflow_id"] == workflow_contract["workflow_id"]
        and trace_artifact["contract_version"] == workflow_contract["contract_version"]
    )
    ...
    bundle = {
        ...
        "provenance": {
            "generation_command": (
                ...
                "workflow_evidence/end_to_end_trace_validation.py"
            ),
            ...
            "validation_trace_artifact_path": str(VALIDATION_TRACE_ARTIFACT_PATH),
        },
        "summary": {
...
def validate_artifact_bundle(bundle):
    ...
    if missing_fields:
        raise ValueError(
            "End-to-end trace bundle is missing required fields: {}".format(
                ", ".join(missing_fields)
            )
        )
    ...
    if bundle["workflow_id"] != WORKFLOW_ID:
        raise ValueError(
            "End-to-end trace bundle has unexpected workflow_id '{}'".format(
                bundle["workflow_id"]
            )
        )
    if bundle["contract_version"] != CONTRACT_VERSION:
        raise ValueError(
            "End-to-end trace bundle has unexpected contract_version '{}'".format(
    ...
    if (
        ...
        raise ValueError(
            "End-to-end trace bundle has inconsistent end-to-end qubit requirements"
        )
    if (
        ...
        raise ValueError(
            "End-to-end trace bundle has inconsistent trace-case requirements"
        )
    if bundle["summary"]["stable_case_ids_present"] is False and bundle["status"] == "pass":
        raise ValueError(
            "End-to-end trace bundle cannot pass without stable mandatory case IDs"
        )
    if (
        ...
        raise ValueError(
            "End-to-end trace bundle cannot pass without matching workflow-contract end-to-end qubit requirements"
        )
    if (
        ...
        raise ValueError(
            "End-to-end trace bundle cannot pass without matching workflow-contract trace-case requirements"
        )
    if (
        ...
        raise ValueError(
            "End-to-end trace bundle cannot pass when workflow thresholds drift from workflow-contract metadata"
        )
    if bundle["summary"]["end_to_end_gate_completed"] != (bundle["status"] == "pass"):
        raise ValueError(
            "End-to-end trace bundle end_to_end_gate_completed summary is inconsistent"
        )
...
def run_validation(
    *,
    ...
    workflow_bundle_path: Path = VALIDATION_WORKFLOW_BASELINE_PATH,
    trace_artifact_path: Path = VALIDATION_TRACE_ARTIFACT_PATH,
    verbose=False,
):
    ...
    return workflow_contract, workflow_bundle, trace_artifact, bundle
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Directory for the end-to-end trace JSON artifact bundle.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--trace-artifact-path",
        type=Path,
        default=VALIDATION_TRACE_ARTIFACT_PATH,
        help="Path to the committed validation trace artifact.",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
