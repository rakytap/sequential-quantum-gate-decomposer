...
| **Partitioning (noisy)**             | `noisy_planner.py`: canonical mixed-state operation surface, validation, partition descriptors (remap + parameter routing). `noisy_runtime.py`: executable partitioned execution, conservative unitary-island fusion vs sequential reference, and the bounded **Phase 3.1** strict/hybrid channel-native follow-on on the frozen counted slice. The current Phase 3.1 closure is a bounded decision study rather than a positive-methods acceleration result; see [`phases/phase-3-1/PRE_PUBLICATION_EVIDENCE_REVIEW_PHASE_3_1.md`](phases/phase-3-1/PRE_PUBLICATION_EVIDENCE_REVIEW_PHASE_3_1.md). |
