...
slice is also present on the required rows, which keeps the current narrative
tied to a real external check without turning the study into a broad simulator
comparison. In addition, the study now includes a full emitted 26-row
whole-workload matrix across the frozen workload families. That matrix does not
support a positive acceleration result. Instead, it shows a bounded decision
pattern: some workloads are already well served by the existing exact fused
baseline, while the richer path remains not yet justified on the rest.
