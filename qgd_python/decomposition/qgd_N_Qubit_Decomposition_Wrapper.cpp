...
create_N_Qubit_Decomposition( Matrix& Umtx, int qbit_num, bool optimize_layer_num, std::map<std::string, int>& config_int, std::map<std::string, double>& config_float, guess_type initial_guess ) {

    return new N_Qubit_Decomposition( Umtx, qbit_num, optimize_layer_num, config_int, config_float, initial_guess );
}
...
qgd_N_Qubit_Decomposition_Wrapper_init(qgd_N_Qubit_Decomposition_Wrapper *self, PyObject *args, PyObject *kwds)
{
    ...
    if (qbit_num > 0 ) {
        self->decomp =  create_N_Qubit_Decomposition( Umtx_mtx, qbit_num, optimize_layer_num, config_int, config_float, qgd_initial_guess);
    }
    else {
