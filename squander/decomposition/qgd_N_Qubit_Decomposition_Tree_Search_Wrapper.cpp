...
create_N_Qubit_Decomposition_Tree_Search( Matrix& Umtx, int qbit_num, std::vector<matrix_base<int>> topology_in, std::map<std::string, Config_Element>& config, int accelerator_num ) {

    return new N_Qubit_Decomposition_Tree_Search( Umtx, qbit_num, topology_in, config, accelerator_num );
}
...
qgd_N_Qubit_Decomposition_Tree_Search_Wrapper_init(qgd_N_Qubit_Decomposition_Tree_Search_Wrapper *self, PyObject *args, PyObject *kwds)
{
    ...
    if (qbit_num > 0 ) {
        try {
            self->decomp = create_N_Qubit_Decomposition_Tree_Search( Umtx_mtx, qbit_num, topology_Cpp, config, accelerator_num);
        }
