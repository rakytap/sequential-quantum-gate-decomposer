...
from squander.partitioning.tools import (
...
def PartitionCircuit(
    ...
    strategy: PartitionStrategy = "kahn",
    filename=None,
) -> tuple[Circuit, np.ndarray, list[int]]:
    """
    ...
    if strategy in [
        ...
        parameters, partitioned_circ, param_order, L = func(
...
def PartitionCircuitQasm(
    filename: str, max_partition_size: int, strategy: PartitionStrategy = "kahn"
) -> tuple[Circuit, np.ndarray, list[int]]:
    """
