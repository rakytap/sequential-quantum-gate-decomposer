from typing import Set, List, Tuple, Dict, Any, Set
import numpy as np
...
def get_float_ops(num_qubit, gate_qubits, control_qubits, is_pure=False, io_penalty=48):
    ...
    return 2 ** (num_qubit - (control_qubits if is_pure else 0)) * (
        g_size * (4 + 2) + 2 * (g_size - 1) + io_penalty
    )
...
def parts_to_float_ops(num_qubit, gate_to_qubit, gate_to_tqubit, allparts):
    ...
    for part in allparts:
        ...
        if gate_to_tqubit is not None:
            ...
            is_pure = (
                len({frozenset(gate_to_qubit[x] - {gate_to_tqubit[x]}) for x in part})
                == 1
            )
            weights.append(
                get_float_ops(
                    num_qubit, len(qubits), len(qubits) - len(tqubits), is_pure
                )
            )
        else:
            weights.append(get_float_ops(num_qubit, len(qubits), 0, False))
    return weights


def total_float_ops(
    num_qubit, max_qubits_per_partition, gate_to_qubit, gate_to_tqubit, allparts
):
    """
    ...
    """
    weights = parts_to_float_ops(
        max_qubits_per_partition, gate_to_qubit, gate_to_tqubit, allparts
    )
    return 2 ** (num_qubit - max_qubits_per_partition) * sum(weights)


def translate_param_order(
    params: np.ndarray, param_order: List[Tuple[int, int, int]]
) -> np.ndarray:
    """
...
def gate_desc_to_gate_index(circ, preparts, qubit_groups_only=False):
    ...
    while S:
        if qubit_groups_only:
            n = next(
                iter(x for x in S if gate_to_qubit[x] <= preparts[len(parts) - 1]), None
            )
        else:
            Scomp = {
                (frozenset(gate_to_qubit[x]), gate_dict[x].get_Name()): x for x in S
            }
        ...
        while n is None:
            ...
            if qubit_groups_only:
                n = next(
                    iter(x for x in S if gate_to_qubit[x] <= preparts[len(parts) - 1]),
                    None,
                )
            else:
                n = next(iter(Scomp.keys() & preparts[len(parts) - 1]), None)
...
def get_qiskit_partitions(filename, max_partition_size):
    ...
    L = [
        [
            (
                frozenset(qc.find_bit(x)[0] for x in dagop.qargs),
                qiskit_to_squander_name(dagop.name),
            )
            for dagop in block
        ]
        for block in blocks
    ...
    partitioned_circ, param_order, parts = kahn_partition_preparts(
        circ, max_partition_size, gate_desc_to_gate_index(circ, L)
    )
...
def get_qiskit_fusion_partitions(filename, max_partition_size):
    ...
    backend = AerSimulator(
        ...
        fusion_enable=True,
        fusion_verbose=True,
        fusion_max_qubit=max_partition_size,
        fusion_threshold=1,
        shots=1,
    )
    ...
    qubits = [
        frozenset(x["qubits"]) for x in meta["output_ops"][:-1]
    ]  # could try to determine control qubits by looking
    from squander.partitioning.kahn import kahn_partition_preparts

    partitioned_circ, param_order, parts = kahn_partition_preparts(
        circ,
        max_partition_size,
        gate_desc_to_gate_index(circ, qubits, qubit_groups_only=True),
    )
    return parameters, partitioned_circ, param_order, parts
...
def get_bqskit_partitions(filename, max_partition_size, partitioner):
    ...
    try:
        ...
        from bqskit.passes.partitioning.cluster import (
            ClusteringPartitioner,
        )  # does a bad job at minimizing partitions
        ...
        L = [
            [
                (
                    frozenset(curloc.location[x] for x in op.location),
                    bqs_to_squander[type(op.gate)],
                )
                for op in curloc.gate._circuit.operations()
            ]
            for curloc in bq_circuit.operations()
        ]
        from squander.partitioning.kahn import kahn_partition_preparts
