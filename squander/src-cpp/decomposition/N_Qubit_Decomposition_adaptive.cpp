...
// U3 gate compression with PBC
void N_Qubit_Decomposition_adaptive::compress_U3_gates_PBC( ) {
...
if ( gates.size() > 0 ) {
    ...
    gate_structure_loc =  static_cast<Gates_block*>(this)->clone()->get_flat_circuit();
}
else {
    std::stringstream sstream;
    sstream << "No circuit initalised." << std::endl;
...
int max_uncompressed_iters = (int)((gate_structure_loc->get_gate_num())/PBC_window);
if ( (int)((gate_structure_loc->get_gate_num())%PBC_window) == 0 ){
    max_uncompressed_iters--;
}
while ( iter<100 && uncompressed_iter_num <= max_uncompressed_iters ) {
    Gates_block* gate_structure_compressed;

    gate_structure_compressed = compress_gate_structure_U3_gates_PBC( gate_structure_loc,uncompressed_iter_num );
    if ( gate_structure_compressed->get_gate_num() < gate_structure_loc->get_gate_num() ) {
        uncompressed_iter_num = 0;
    }
    else {
    ...
    if ( gate_structure_compressed != gate_structure_loc ) {
        ...
        gate_structure_loc = gate_structure_compressed;
        gate_structure_compressed = NULL;
        max_uncompressed_iters = (int)((gate_structure_loc->get_gate_num())/PBC_window);
        if ( (int)((gate_structure_loc->get_gate_num())%PBC_window) == 0 ){
        max_uncompressed_iters--;
        }
...
Gates_block*
N_Qubit_Decomposition_adaptive::compress_gate_structure_U3_gates_PBC( Gates_block* gate_structure, int uncompressed_iter_num ){
    int layer_num_max;
    ...
    // create a list of layers to be tested for removal.
    std::vector<double> layers_parameters(layer_num_orig,15.0);
    std::vector<int> layers_idx_sorted(layer_num_orig,-1);
    //layers_to_remove.reserve(layer_num_orig);
    for (int idx=0; idx<layer_num_orig;idx++){
        Gate* gate = gate_structure->get_gate(idx);
        if ( gate->get_name() == "U3" ) {
            layers_parameters[idx] = calculate_U3_score(gate, optimized_parameters_mtx);
            layers_idx_sorted[idx] = idx;
        }
        else {
            layers_idx_sorted[idx] = -1;
        }
    }
    ...
    for (int idx=0; idx<layer_num_max; idx++ ) { 
        if (layers_idx_sorted[uncompressed_iter_num*PBC_window+idx] == -1) {
            continue;
        }
    ...
    for (int idx=0; idx<panelties_num; idx++) {
       ...
       /* }
        ...
        panelties[idx]                = gate_structure_tmp->get_gate_num();
        gate_structures_vec[idx]      = gate_structure_tmp;
        current_minimum_vec[idx]      = current_minimum_loc;
        iteration_num_vec[idx]        = iteration_num; // the accumulated number of optimization iterations
...
double N_Qubit_Decomposition_adaptive::calculate_U3_score(Gate* gate, Matrix_real& optimized_parameters) {
    double score = 0.0;
    double parameter = optimized_parameters[gate->get_parameter_start_idx()];
    double parameter_2 = optimized_parameters[gate->get_parameter_start_idx()+1];
    double parameter_3 = optimized_parameters[gate->get_parameter_start_idx()+2];

    score = std::sin(parameter)*std::sin(parameter) + std::sin(parameter_2+parameter_3)*std::sin(parameter_2+parameter_3);
