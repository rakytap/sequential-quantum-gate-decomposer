...
*/
/*! \file SXdg.cpp
    \brief Class representing a SXdg gate.
*/
...
*/
SXdg::SXdg(int qbit_num_in, int target_qbit_in) {
...
*/
Matrix
SXdg::get_matrix( int parallel) {

        Matrix SXdg_matrix = create_identity(matrix_size);
        apply_to(SXdg_matrix, parallel);

#ifdef DEBUG
        if (SXdg_matrix.isnan()) {
            std::stringstream sstream;
	    sstream << "SXdg::get_matrix: SXdg_matrix contains NaN." << std::endl;
            print(sstream, 1);	  
        }
#endif
...
*/
void 
SXdg::apply_to( Matrix& input, int parallel ) {

    if (input.rows != matrix_size ) {
        std::string err("SXdg::apply_to: Wrong input size in SXdg gate apply.");
        throw err;    
    }
    ...
    //apply_kernel_to function to SXdg gate 
...
*/
void 
SXdg::apply_from_right( Matrix& input ) {
    ...
    //apply_kernel_from_right function to SXdg gate 
    ...
    while ( current_idx_pair < matrix_size ) {
        ...
        tbb::parallel_for(0, index_step, 1, [&](int idx) {  
            ...
            for ( int row_idx=0; row_idx<matrix_size; row_idx++) {
                ...
                QGD_Complex16 tmp1 = mult(SXdg_1qbit[0], element);
                QGD_Complex16 tmp2 = mult(SXdg_1qbit[2], element_pair);
                input[index].real = tmp1.real + tmp2.real;
                input[index].imag = tmp1.imag + tmp2.imag;

                tmp1 = mult(SXdg_1qbit[1], element);
                tmp2 = mult(SXdg_1qbit[3], element_pair);
...
*/
void SXdg::reorder_qubits( std::vector<int> qbit_list) {
...
*/
void SXdg::set_qbit_num(int qbit_num_in) {
