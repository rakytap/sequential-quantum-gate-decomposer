...
/**
@brief Call to solve layer by layer the optimization problem via the AGENT algorithm. The optimalized parameters are
stored in attribute optimized_parameters.
...
void Optimization_Interface::solve_layer_optimization_problem_AGENTS(int num_of_parameters,
    ...
    if ((((cost_fnc != FROBENIUS_NORM) && (cost_fnc != HILBERT_SCHMIDT_TEST)) && cost_fnc != VQE) &&
        (cost_fnc != INFIDELITY) && (cost_fnc != GQML)) {
        std::string err("Optimization_Interface::solve_layer_optimization_problem_AGENTS: Only cost functions 0 and 3 "
                        "are implemented");
        throw err;
    }
    ...
    for (int agent_idx = 0; agent_idx < agent_num; agent_idx++) {
        ...
        if (current_rank == 0) {
            ...
            if (agent_idx == 0) {
                memcpy(solution_guess_mtx_agent.get_data(), solution_guess.get_data(),
                       solution_guess.size() * sizeof(double));
            } else {
    ...
    for (long long iter_idx = 0; iter_idx < max_inner_iterations_loc; iter_idx++) {
        ...
        if (three_point_line_search) {
            ...
            for (int agent_idx = 0; agent_idx < agent_num; agent_idx++) {
                ...
                Matrix_real& solution_guess_mtx_agent = solution_guess_mtx_agents[agent_idx];
                solution_guess_mtx_agent[param_idx_agents[agent_idx]] =
                    parameter_value_save_agents[agent_idx] + parameter_shift;
            ...
            for (int agent_idx = 0; agent_idx < agent_num; agent_idx++) {
                ...
                */
                ...
                double f4 = f0_shifted_pi4 - offset -
                            gamma * sin(parameter_value_save_agents[agent_idx] + M_PI_quarter + varphi);
                ...
                Matrix_real& solution_guess_mtx_agent = solution_guess_mtx_agents[agent_idx];
                solution_guess_mtx_agent[param_idx_agents[agent_idx]] =
                    parameter_value_save_agents[agent_idx] + parameter_shift[0];
            ...
            for (int idx = 0; idx < agent_num; idx++) {
                Matrix_real& solution_guess_mtx_agent = solution_guess_mtx_agents[idx];
                solution_guess_mtx_agent[param_idx_agents[idx]] += M_PI_eight;
            }

            Matrix_real f0_shifted_pi8_agents = optimization_problem_batched(solution_guess_mtx_agents);
            ...
            Matrix_real f0_shifted_pi4_agents = optimization_problem_batched(solution_guess_mtx_agents);

            for (int idx = 0; idx < agent_num; idx++) {
                Matrix_real& solution_guess_mtx_agent = solution_guess_mtx_agents[idx];
                solution_guess_mtx_agent[param_idx_agents[idx]] += M_PI_quarter;
            }

            Matrix_real f0_shifted_pi2_agents = optimization_problem_batched(solution_guess_mtx_agents);
            ...
            Matrix_real f0_shifted_3pi4_agents = optimization_problem_batched(solution_guess_mtx_agents);
            for (int idx = 0; idx < agent_num; idx++) {
                ...
                double f4 = f0_shifted_pi8 - C - B * cos(2 * parameter_value_save_agents[idx] + M_PI_quarter + delta);
                ...
                Matrix_real parameter_shift(1, 1);
                if (abs(B) > abs(A)) {
                    parameter_shift[0] = M_PI_half - delta / 2 - parameter_value_save_agents[idx] / 2;
                } else {
                    parameter_shift[0] = M_PI_quarter - phi / 4 - parameter_value_save_agents[idx] / 4;
                }
                parameter_shift[0] = std::fmod(parameter_shift[0], M_PI_double);
                ...
                Matrix_real& solution_guess_mtx_agent = solution_guess_mtx_agents[idx];
                solution_guess_mtx_agent[param_idx_agents[idx]] = parameter_value_save_agents[idx] + parameter_shift[0];

                current_minimum_agents[idx] = f;
            }

        } else {
            std::string err("solve_layer_optimization_problem_AGENTS: Not implemented method.");
            throw err;
        }
        ...
        /*
                    ...
                    if ( iter_idx % agent_lifetime_loc == 0 ) {
                        ...
                        for( int agent_idx=0; agent_idx<agent_num; agent_idx++ ) {
                            double prob_loc = exp( (current_minimum_agents_min -
           current_minimum_agents[agent_idx])*40.0/current_minimum_agents_min ); agent_probs[agent_idx] = prob_loc;
        ...
        */
        ...
        for (int agent_idx = 0; agent_idx < agent_num; agent_idx++) {
            ...
            if (current_minimum_agent < optimization_tolerance_loc) {
                ...
                memcpy(optimized_parameters_mtx.get_data(), solution_guess_mtx_agent.get_data(),
            ...
            if (iter_idx % agent_lifetime_loc == 0) {
                ...
                if (current_minimum_agent <= current_minimum) {
                    ...
                    // less successful agent migh choose to keep their current state, or choose the state of more
                    // successful agents
                    ...
                    if (current_rank == 0) {
                        ...
                        if (random_num < agent_exploration_rate && agent_idx != most_successfull_agent) {
                            ...
                            std::stringstream sstream;
                            sstream << "agent " << agent_idx << ": adopts the state of the most succesful agent. "
                                    << most_successfull_agent << std::endl;
                            print(sstream, 5);

                            current_minimum_agents[agent_idx] = current_minimum_agents[most_successfull_agent];
                            memcpy(solution_guess_mtx_agent.get_data(),
                                   solution_guess_mtx_agents[most_successfull_agent].get_data(),
                                   solution_guess_mtx_agent.size() * sizeof(double));

                            random_num = random_numbers[agent_idx * random_numbers.stride + 1];

                            if (random_num < agent_randomization_rate_loc) {
                                randomize_parameters(optimized_parameters_mtx, solution_guess_mtx_agent,
                                                     current_minimum);
                    ...
                    MPI_Bcast((void*)solution_guess_mtx_agent.get_data(), num_of_parameters, MPI_DOUBLE, 0,
                              MPI_COMM_WORLD);
                ...
                if (agent_idx == 0) {
                    ...
                    current_minimum_mean =
                        current_minimum_mean +
                        (current_minimum - current_minimum_vec[current_minimum_idx]) / current_minimum_vec.size();
                    ...
                    for (int idx = 0; idx < current_minimum_vec.size(); idx++) {
                        var_current_minimum =
                            var_current_minimum + (current_minimum_vec[idx] - current_minimum_mean) *
                                                      (current_minimum_vec[idx] - current_minimum_mean);
                    }
            ...
            if (iter_idx % agent_lifetime_loc == 0 && agent_idx == 0) {
                std::stringstream sstream;
                sstream << "AGENTS, agent " << agent_idx << ": processed iterations "
                        << (double)iter_idx / max_inner_iterations_loc * 100 << "\%";
                sstream << ", current minimum of agent 0: " << current_minimum_agents[0]
                        << " global current minimum: " << current_minimum << " CPU time: " << CPU_time;
...
/**
@brief Call to solve layer by layer the optimization problem via the AGENT COMBINED algorithm. The optimalized
parameters are stored in attribute optimized_parameters.
@param num_of_parameters Number of parameters to be optimized
@param solution_guess Array containing the solution guess.
*/
void Optimization_Interface::solve_layer_optimization_problem_AGENTS_COMBINED(int num_of_parameters,
    ...
    for (int loop_idx = 0; loop_idx < 1; loop_idx++) {
        ...
        memcpy(solution_guess_AGENTS.get_data(), optimized_parameters_mtx.get_data(),
        ...
        memcpy(solution_guess_COSINE.get_data(), optimized_parameters_mtx.get_data(),
               optimized_parameters_mtx.size() * sizeof(double));
