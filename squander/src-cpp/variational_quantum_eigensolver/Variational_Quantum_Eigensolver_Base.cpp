...
const char *density_noise_type_to_label(density_noise_type noise_type) {
  switch (noise_type) {
...
parse_vqe_backend_mode(std::map<std::string, Config_Element> &config) {
  ...
  default:
    throw std::string(
        "Variational_Quantum_Eigensolver_Base::Variational_Quantum_Eigensolver_"
        "Base: unsupported backend_mode value");
  }
}
...
void Variational_Quantum_Eigensolver_Base::set_density_noise_specs(
  ...
  for (const auto &spec : density_noise_specs_in) {
    if (spec.target_qbit < 0 || spec.target_qbit >= qbit_num) {
      throw std::string("Variational_Quantum_Eigensolver_Base::set_density_"
                        "noise_specs: target_qbit out of range");
    }

    if (spec.after_gate_index < 0) {
      throw std::string("Variational_Quantum_Eigensolver_Base::set_density_"
                        "noise_specs: after_gate_index must be non-negative");
    }

    if (spec.value < 0.0 || spec.value > 1.0) {
      throw std::string("Variational_Quantum_Eigensolver_Base::set_density_"
                        "noise_specs: noise values must be in [0, 1]");
    }
  }
...
void Variational_Quantum_Eigensolver_Base::validate_density_anchor_support(
  ...
  if (backend_mode == STATE_VECTOR_BACKEND) {
    if (!density_noise_specs.empty()) {
      throw std::string("Variational_Quantum_Eigensolver_Base::validate_"
                        "density_anchor_support: state_vector backend does not "
                        "support density_matrix-only noise configuration");
    }
    return;
  }

  int expected_dimension = Power_of_2(qbit_num);
  if (Hamiltonian.rows != expected_dimension ||
      Hamiltonian.cols != expected_dimension) {
    throw std::string("Variational_Quantum_Eigensolver_Base::validate_density_"
                      "anchor_support: density_matrix backend requires a "
                      "square Hamiltonian matching the configured qubit count");
  }

  if (require_gradient_support) {
    throw std::string(
        "Variational_Quantum_Eigensolver_Base::validate_density_anchor_support:"
        " density_matrix backend does not support gradient-based optimization");
  }

  if (ansatz != HEA) {
    throw std::string(
        "Variational_Quantum_Eigensolver_Base::validate_density_anchor_support:"
        " density_matrix backend currently supports only the HEA ansatz");
  }

  if (require_optimizer_support && !density_optimizer_supported()) {
    throw std::string("Variational_Quantum_Eigensolver_Base::validate_density_"
                      "anchor_support: density_matrix backend currently "
  ...
  if (circuit_source == VQE_CIRCUIT_SOURCE_UNSET) {
    throw std::string(
        "Variational_Quantum_Eigensolver_Base::validate_density_anchor_support:"
        " density_matrix backend requires a generated HEA circuit");
  }

  if (circuit_source != VQE_CIRCUIT_SOURCE_GENERATED_HEA) {
    std::string error(
        "Variational_Quantum_Eigensolver_Base::validate_density_anchor_support:"
        " unsupported circuit source in density backend path: ");
  ...
  for (int gate_idx = 0; gate_idx < gate_num; ++gate_idx) {
    ...
    default: {
      std::string error(
          "Variational_Quantum_Eigensolver_Base::validate_density_anchor_"
          "support: first unsupported gate in density backend path is ");
      error += gate->get_name();
      throw error;
    }
    }
  ...
  for (size_t spec_idx = 0; spec_idx < density_noise_specs.size(); ++spec_idx) {
    ...
    if (spec.after_gate_index >= gate_num) {
      std::string error(
          "Variational_Quantum_Eigensolver_Base::validate_density_anchor_"
          "support: unsupported density-noise insertion at index ");
...
void Variational_Quantum_Eigensolver_Base::append_density_noise_for_gate_index(
  ...
  for (const auto &spec : density_noise_specs) {
    ...
    default: {
      std::string error(
          "Variational_Quantum_Eigensolver_Base::append_density_noise_for_gate_"
          "index: unsupported density-noise type ");
...
void Variational_Quantum_Eigensolver_Base::
  ...
  for (int gate_idx = 0; gate_idx < gate_num; ++gate_idx) {
    ...
    default: {
      std::string error(
          "Variational_Quantum_Eigensolver_Base::lower_anchor_circuit_to_noisy_"
          "circuit: unsupported gate in density backend path: ");
      error += gate->get_name();
      throw error;
    }
    }
...
double
Variational_Quantum_Eigensolver_Base::expectation_value_of_density_energy_real(
    const squander::density::DensityMatrix &rho) const {

  if (Hamiltonian.rows != rho.get_dim() || Hamiltonian.cols != rho.get_dim()) {
    throw std::string("Variational_Quantum_Eigensolver_Base::expectation_value_"
                      "of_density_energy_real: dimension mismatch");
  }
  ...
  if (!std::isfinite(energy_real)) {
    throw std::string("Variational_Quantum_Eigensolver_Base::expectation_value_"
                      "of_density_energy_real: produced a non-finite energy");
  }
...
Variational_Quantum_Eigensolver_Base::Variational_Quantum_Eigensolver_Base(
    ...
    : Optimization_Interface(Matrix(Power_of_2(qbit_num_in), 1), qbit_num_in,
                             false, config_in, RANDOM, accelerator_num) {
  ...
  // override optimization parameters governing the convergence used in gate
...
void Variational_Quantum_Eigensolver_Base::start_optimization() {
  ...
  if (gates.size() == 0) {
    std::string error("Variational_Quantum_Eigensolver_Base::Get_ground_state: "
                      "for VQE process the circuit needs to be initialized");
    throw error;
  }
  ...
  if (num_of_parameters == 0) {
    std::string error("Variational_Quantum_Eigensolver_Base::Get_ground_state: "
                      "No intial parameters were given");
    throw error;
  }

  if (num_of_parameters != get_parameter_num()) {
    std::string error("Variational_Quantum_Eigensolver_Base::Get_ground_state: "
                      "The number of initial parameters does not match with "
...
*/
QGD_Complex16 Variational_Quantum_Eigensolver_Base::Expectation_value_of_energy(
    Matrix &State_left, Matrix &State_right) {

  if (State_left.rows != State_right.rows ||
      Hamiltonian.rows != State_right.rows) {
    std::string error("Variational_Quantum_Eigensolver_Base::Expectation_value_"
                      "of_energy: States on the right and left should be of "
                      "the same dimension as the Hamiltonian");
    throw error;
  }
  ...
  tbb::parallel_for(
      tbb::blocked_range<int>(0, State_left.rows, 1024),
      [&](tbb::blocked_range<int> r) {
        double &energy_local_real = priv_partial_energy_real.local();
        double &energy_local_imag = priv_partial_energy_imag.local();

        for (int idx = r.begin(); idx < r.end(); idx++) {
          energy_local_real += State_left[idx].real * tmp[idx].real +
                               State_left[idx].imag * tmp[idx].imag;
          energy_local_imag += State_left[idx].real * tmp[idx].imag -
                               State_left[idx].imag * tmp[idx].real;
        }
      });

  // calculate the final cost function
  priv_partial_energy_real.combine_each(
      [&Energy](double a) { Energy.real = Energy.real + a; });

  priv_partial_energy_imag.combine_each(
      [&Energy](double a) { Energy.imag = Energy.imag + a; });

  tmp.release_data(); // TODO: this is not necessary, beacause it is called with
...
@return The calculated expectation value
*/
double Variational_Quantum_Eigensolver_Base::Expectation_value_of_energy_real(
    Matrix &State_left, Matrix &State_right) {

  if (State_left.rows != State_right.rows ||
      Hamiltonian.rows != State_right.rows) {
    std::string error("Variational_Quantum_Eigensolver_Base::Expectation_value_"
                      "of_energy_real: States on the right and left should be "
                      "of the same dimension as the Hamiltonian");
    throw error;
  }
  ...
  tbb::parallel_for(tbb::blocked_range<int>(0, State_left.rows, 1024),
                    [&](tbb::blocked_range<int> r) {
                      double &energy_local = priv_partial_energy.local();

                      for (int idx = r.begin(); idx < r.end(); idx++) {
                        energy_local += State_left[idx].real * tmp[idx].real +
                                        State_left[idx].imag * tmp[idx].imag;
                      }
                    });

  // calculate the final cost function
  priv_partial_energy.combine_each(
      [&Energy](double a) { Energy = Energy + a; });
 ...
 *  5. Compute sample mean, variance and standard error (std_error =
 * sqrt(var/shots)).
 *
 * Parameters
 * ----------
 ...
 */
SimulationResult
Variational_Quantum_Eigensolver_Base::Expectation_value_with_shot_noise_real(
    Matrix &State, int shots, uint64_t seed, double p_readout) {

  if (State.rows != Hamiltonian.rows) {
    throw std::string(
        "Expectation_value_with_shot_noise_real: dimension mismatch");
  }
  ...
  if ((1 << n_qubits) != N) {
    throw std::string(
        "Expectation_value_with_shot_noise_real: State.rows not power of two");
  }
  ...
  std::discrete_distribution<int>
      distX; // will be initialized if has_xx == true
  ...
  if (has_xx) {
    ...
    for (const auto &t : xx_terms) {
      ...
      if (i < 0 || i >= n_qubits || j < 0 || j >= n_qubits) {
        throw std::string("XX term index out of range in "
    ...
    if (sum_pX <= 0.0) {
      throw std::string("X-rotated state has zero norm in "
                        "Expectation_value_with_shot_noise_real");
    }
    for (auto &v : probsX)
      v /= sum_pX;
  ...
  std::discrete_distribution<int>
      distY; // will be initialized if has_yy == true
  ...
  if (has_yy) {
    ...
    for (const auto &t : yy_terms) {
      ...
      std::tie(i, j, coeff) = t;
      if (i < 0 || i >= n_qubits || j < 0 || j >= n_qubits) {
        throw std::string("YY term index out of range in "
                          "Expectation_value_with_shot_noise_real");
      }
      need_SDG_H[i] = true;
      need_SDG_H[j] = true;
    ...
    // Build Y-basis probabilities (after S†H rotation, Z-eigenvalues encode
    // Y-eigenvalues)
    ...
    if (sum_pY <= 0.0) {
      throw std::string("Y-rotated state has zero norm in "
                        "Expectation_value_with_shot_noise_real");
    }
    for (auto &v : probsY)
      v /= sum_pY;
  ...
  const double std_error =
      (variance > 0.0) ? std::sqrt(variance / static_cast<double>(shots)) : 0.0;
...
*/
double Variational_Quantum_Eigensolver_Base::optimization_problem_non_static(
    Matrix_real parameters, void *void_instance) {

  double Energy = 0.0;

  Variational_Quantum_Eigensolver_Base *instance =
      reinterpret_cast<Variational_Quantum_Eigensolver_Base *>(void_instance);
...
#ifdef __GROQ__
/**
@brief The optimization problem of the final optimization implemented to be run
on Groq hardware
@param parameters An array of the free parameters to be optimized.
@param chosen_device Indicate the device on which the state vector emulation is
performed
@return Returns with the cost function.
*/
double Variational_Quantum_Eigensolver_Base::optimization_problem_Groq(
    Matrix_real &parameters, int chosen_device) {
  ...
  extract_gate_kernels_target_and_control_qubits(u3_qbit, target_qbits,
                                                 control_qbits, parameters);

  // initialize the initial state on the chip if it was not given
  if (initial_state.size() == 0) {

    Matrix State_zero(0, 0);
    apply_to_groq_sv(accelerator_num, chosen_device, qbit_num, u3_qbit,
                     target_qbits, control_qbits, State_zero, id);
    ...
    apply_to_groq_sv(accelerator_num, chosen_device, qbit_num, u3_qbit,
                     target_qbits, control_qbits, State, id);
  }
  ...
  /*
      ...
      for( int64_t idx=0; idx<State_copy.size(); idx++ ) {
          ...
          double diff_increment = element_diff.real*element_diff.real +
     element_diff.imag*element_diff.imag; diff = diff + diff_increment;
      }

      std::cout << "Variational_Quantum_Eigensolver_Base::apply_to checking
     diff: " << diff << std::endl;
...
void Variational_Quantum_Eigensolver_Base::
  ...
  tbb::parallel_invoke(
      ...
      [&] {
        ...
        State_deriv =
            instance->apply_derivate_to(parameters, State_loc, parallel);
        State_loc.release_data();
      });

  tbb::parallel_for(tbb::blocked_range<int>(0, parameter_num_loc, 2),
                    [&](tbb::blocked_range<int> r) {
                      for (int idx = r.begin(); idx < r.end(); ++idx) {
                        grad[idx] =
                            2 * instance->Expectation_value_of_energy_real(
                                    State_deriv[idx], State);
                      }
                    });
  ...
  tbb::parallel_for( tbb::blocked_range<int>(0,parameter_num_loc,1),
  [&](tbb::blocked_range<int> r) { for (int idx=r.begin(); idx<r.end(); ++idx) {
          Matrix_real param_tmp = parameters.copy();
          param_tmp[idx] += delta;

          Matrix State = instance->initial_state.copy();
          instance->apply_to(param_tmp, State);
          double f_loc = instance->Expectation_value_of_energy_real(State,
  State);
...
@param grad Array containing the calculated gradient components.
*/
void Variational_Quantum_Eigensolver_Base::optimization_problem_combined(
    Matrix_real parameters, double *f0, Matrix_real grad) {
...
@param grad Array containing the calculated gradient components.
*/
void Variational_Quantum_Eigensolver_Base::optimization_problem_grad_vqe(
    Matrix_real parameters, void *void_instance, Matrix_real &grad) {

  double f0;
  Variational_Quantum_Eigensolver_Base *instance =
      reinterpret_cast<Variational_Quantum_Eigensolver_Base *>(void_instance);
  instance->optimization_problem_combined_non_static(parameters, void_instance,
                                                     &f0, grad);
  return;
}
...
*/
double
Variational_Quantum_Eigensolver_Base::optimization_problem(double *parameters) {
...
void Variational_Quantum_Eigensolver_Base::initialize_zero_state() {
  ...
  memset(initial_state.get_data() + 2, 0,
         (initial_state.size() * 2 - 2) * sizeof(double));
...
void Variational_Quantum_Eigensolver_Base::generate_circuit(int layers,
  ...
  case HEA: {
    ...
    if (qbit_num < 1) {
      std::string error(
          "Variational_Quantum_Eigensolver_Base::generate_initial_circuit: "
          "number of qubits should be at least 1");
      throw error;
    }
    ...
    for (int layer_idx = 0; layer_idx < layers; layer_idx++) {
      ...
      for (int control_qbit = 1; control_qbit < qbit_num - 1;
           control_qbit = control_qbit + 2) {
  ...
  case HEA_ZYZ: {
    ...
    if (qbit_num < 2) {
      std::string error(
          "Variational_Quantum_Eigensolver_Base::generate_initial_circuit: "
          "number of qubits should be at least 2");
      throw error;
    }
    ...
    for (int layer_idx = 0; layer_idx < layers; layer_idx++) {
      ...
      for (int idx = 0; idx < inner_blocks; idx++) {
        ...
        // single qubit gates in a ablock are then merged into a single gate
        // kernel.
        block_1->add_rz(1);
        block_1->add_ry(1);
        block_1->add_rz(1);
  ...
  default:
    std::string error("Variational_Quantum_Eigensolver_Base::generate_initial_"
                      "circuit: ansatz not implemented");
    throw error;
  }
}

void Variational_Quantum_Eigensolver_Base::set_gate_structure(
    std::string filename) {

  release_gates();
  Matrix_real optimized_parameters_mtx_tmp;
  Gates_block *gate_structure_tmp =
      import_gate_list_from_binary(optimized_parameters_mtx_tmp, filename);
  ...
  if (gates.size() > 0) {
    ...
    Matrix_real optimized_parameters_mtx_tmp2(
        1,
        optimized_parameters_mtx_tmp.size() + optimized_parameters_mtx.size());
    memcpy(optimized_parameters_mtx_tmp2.get_data(),
           optimized_parameters_mtx_tmp.get_data(),
           optimized_parameters_mtx_tmp.size() * sizeof(double));
    memcpy(optimized_parameters_mtx_tmp2.get_data() +
               optimized_parameters_mtx_tmp.size(),
           optimized_parameters_mtx.get_data(),
           optimized_parameters_mtx.size() * sizeof(double));
...
void Variational_Quantum_Eigensolver_Base::set_custom_gate_structure(
...
*/
void Variational_Quantum_Eigensolver_Base::set_initial_state(
    Matrix initial_state_in) {

  // check the size of the input state
  if (initial_state_in.size() != 1 << qbit_num) {
    std::string error("Variational_Quantum_Eigensolver_Base::set_initial_state:"
                      " teh number of elements in the input state does not "
                      "match with the number of qubits.");
    throw error;
  }

  initial_state = Matrix(1 << qbit_num, 1);
  memcpy(initial_state.get_data(), initial_state_in.get_data(),
         initial_state_in.size() * sizeof(QGD_Complex16));
}
