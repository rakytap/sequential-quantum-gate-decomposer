...
class qgd_SABRE:
    def __init__(
        ...
        self.possible_connections_control, self.neighbours = (
            self.generate_possible_connections()
        )
    ...
    def _compute_smart_initial_layout(self, circuit):
        ...
        remaining_logical = [
            q for q in range(self.circuit_qbit_num) if q not in placed_logical
        ]
    ...
    def obtain_SWAPS(self, F, pi, DAG):
        ...
        for qbit in involved_qbits:
            ...
            for Q_cand in self.neighbours[Q]:
                ...
                swap = (min(qbit, int(q_cand)), max(qbit, int(q_cand)))
                swaps.add(swap)
            ...
            while lookahead_count < self.max_lookahead and children:
                for gate_idx in children[:]:  # Copy to avoid modification issues
                    ...
                    if gate.get_Control_Qbit() == -1:
                        ...
                        gate_depth = self.calculate_gate_depth(
                            gate_idx, IDAG, resolved_gates
                        )
                        if (
                            len(E) < self.max_E_size
                            and (
                                gate.get_Target_Qbit() in involved_qbits
                                or gate.get_Control_Qbit() in involved_qbits
                            )
                            and gate_idx not in E_set
                            and gate_depth < 6
                        ):
                            E.append(gate_idx)
                            E_set.add(gate_idx)
    ...
    def Heuristic_search(self, F, pi, DAG, IDAG):
        ...
        while len(F) != 0:
            ...
            for gate_idx in F:
                gate = DAG[gate_idx][0]
                possible, flag = self.is_gate_possible(
                    pi, gate.get_Target_Qbit(), gate.get_Control_Qbit()
                )
                if possible:
            ...
            if len(execute_gate_list) != 0:
                ...
                for gate_idx in execute_gate_list:
                    ...
                    for new_gate_idx in successors:
                        resolved = self.check_dependencies(
                            new_gate_idx, IDAG, resolved_gates
                        )
                ...
                if len(swaps) != 0:
                    for SWAP in swaps:
                        ...
                        score = self.score_swap_improved(
                            SWAP, F, E, pi, DAG, IDAG, resolved_gates
                        )
                        scores.append(score)
        ...
        for gate_idx in gate_order:
            if isinstance(gate_idx, int):
                ...
                if gate.get_Parameter_Num() != 0:
                    ...
                    params = parameters[
                        start_idx : start_idx + gate.get_Parameter_Num()
                    ]
    ...
    def map_circuit(self, parameters=np.array([]), iter_num=1):
        ...
        for idx in range(iter_num):
            ...
            self.Heuristic_search(
                first_layer_reverse.copy(), init_pi, DAG_reverse, IDAG_reverse
            )

        final_pi = init_pi.copy()
        gate_order, flags, swap_count = self.Heuristic_search(
            first_layer.copy(), final_pi, DAG, IDAG
        )
        circuit_mapped, parameters_new = self.get_mapped_circuit(
            circuit, init_pi.copy(), gate_order, flags, parameters
        )
