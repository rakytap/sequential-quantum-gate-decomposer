...
def test_phase3_story1_continuity_surface_matches_bridge_metadata(qbit_num):
...
def test_phase3_story1_rejects_non_partitioned_density_mode():
...
def test_phase3_story1_rejects_bridge_noise_with_invalid_after_gate_index():
    vqe, _, _ = build_phase3_story1_continuity_vqe(4)
    bridge = vqe.describe_density_bridge()
    bridge["operations"] = [
        {
...
def test_phase3_story2_microcases_share_canonical_schema():
    reference_keys = None
    operation_keys = None
...
def test_phase3_story2_structured_families_share_canonical_schema():
    structured_cases = list(iter_story2_structured_surfaces())
...
def test_phase3_story2_workload_ids_are_unique():
    workload_ids = set()
...
def test_phase3_story3_audit_record_tracks_continuity_provenance():
    vqe, _, _ = build_phase3_story1_continuity_vqe(4)
    surface = build_phase3_continuity_planner_surface(vqe)
...
def test_phase3_story3_audit_record_tracks_structured_workload_provenance():
    metadata, surface = next(iter(iter_story2_structured_surfaces()))
...
def test_phase3_story4_exact_qgd_circuit_lowering_preserves_order_and_param_spans():
    circuit = Circuit(2)
    circuit.add_U3(0)
    circuit.add_U3(1)
...
def test_phase3_story4_legacy_lowering_accepts_explicit_supported_noise():
    circuit = Circuit(2)
    circuit.add_U3(0)
    circuit.add_CNOT(1, 0)
...
def test_phase3_story4_rejects_unsupported_legacy_gate_family():
    circuit = Circuit(1)
    circuit.add_H(0)
...
def test_phase3_story4_rejects_invalid_legacy_noise_schedule():
    circuit = Circuit(2)
    circuit.add_U3(0)
    circuit.add_CNOT(1, 0)
...
def test_phase3_story5_preflight_accepts_supported_bridge_request():
    vqe, _, _ = build_phase3_story1_continuity_vqe(4)
    bridge = vqe.describe_density_bridge()
...
def test_phase3_story5_preflight_rejects_unsupported_source_type():
    with pytest.raises(
        NoisyPlannerValidationError,
...
def test_phase3_story5_preflight_rejects_missing_source_payload():
    with pytest.raises(
        NoisyPlannerValidationError,
...
def test_phase3_story5_preflight_rejects_unsupported_noise_model_in_operation_specs():
...
def test_phase3_story5_preflight_rejects_disallowed_mode_claim():
    microcase = mandatory_microcase_definitions()[0]
    with pytest.raises(
        NoisyPlannerValidationError,
