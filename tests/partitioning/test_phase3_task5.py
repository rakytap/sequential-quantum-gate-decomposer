...
from benchmarks.density_matrix.planner_calibration.common import (
    ...
    TASK5_PLANNER_FAMILY_SPAN_BUDGET,
...
def test_phase3_task5_story1_candidate_enumeration_is_deterministic():
    candidates = build_task5_planner_candidates()
...
def test_phase3_task5_story1_candidate_audit_covers_required_workload_kinds():
...
def test_phase3_task5_story1_bundle_core_fields_are_stable():
    bundle = build_story1_bundle(build_story1_cases())
    assert bundle["status"] == "pass"
    assert bundle["candidate_schema_version"] == TASK5_CANDIDATE_SCHEMA_VERSION
...
def test_phase3_task5_story2_workload_matrix_covers_required_case_inventory():
...
def test_phase3_task5_story2_bundle_core_fields_are_stable():
...
def test_phase3_task5_story3_density_signal_surface_is_present_and_ranked():
...
def test_phase3_task5_story3_density_score_changes_with_noise_pattern():
    cases = build_story3_cases()
...
def test_phase3_task5_story3_bundle_core_fields_are_stable():
    bundle = build_story3_bundle(build_story3_cases())
    assert bundle["status"] == "pass"
    assert bundle["signal_schema_version"] == "phase3_task5_density_signal_v1"
...
def test_phase3_task5_story4_correctness_gate_marks_full_matrix_counted(story4_cases):
...
def test_phase3_task5_story4_bundle_core_fields_are_stable(story4_cases):
    bundle = build_story4_bundle(story4_cases)
    assert bundle["status"] == "pass"
    assert bundle["record_schema_version"] == "phase3_task5_calibration_record_v1"
...
@pytest.fixture(scope="module")
def story5_payload():
    return build_task5_claim_selection_payload()


def test_phase3_task5_story5_selects_one_supported_candidate(story5_payload):
...
def test_phase3_task5_story5_bundle_core_fields_are_stable():
    bundle = build_story5_bundle()
    assert bundle["status"] == "pass"
    assert bundle["claim_selection_schema_version"] == "phase3_task5_claim_selection_v1"
...
def test_phase3_task5_story6_shared_bundle_is_consumer_stable():
...
def test_phase3_task5_story6_bundle_core_fields_are_stable():
    bundle = build_story6_bundle()
    assert bundle["status"] == "pass"
    assert bundle["calibration_bundle_schema_version"] == "phase3_task5_calibration_bundle_v1"
    assert bundle["summary"]["total_cases"] == 75


def test_phase3_task5_story7_boundary_surface_is_explicit():
...
def test_phase3_task5_story7_bundle_core_fields_are_stable():
    bundle = build_story7_bundle()
    assert bundle["status"] == "pass"
    assert bundle["boundary_schema_version"] == "phase3_task5_claim_boundary_v1"
    assert bundle["summary"]["selected_candidate_id"] in {
        "span_budget_q2",
        "span_budget_q3",
        "span_budget_q4",
