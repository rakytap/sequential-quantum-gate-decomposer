...
def test_phase3_task6_story1_matrix_covers_required_inventory():
...
def test_phase3_task6_story1_bundle_core_fields_are_stable():
    bundle = build_story1_bundle(build_story1_cases())
    assert bundle["status"] == "pass"
    assert bundle["record_schema_version"] == TASK6_CASE_SCHEMA_VERSION
...
def test_phase3_task6_story2_internal_gate_passes_full_matrix(story2_cases):
    ...
    assert all(case["record_schema_version"] == TASK6_CASE_SCHEMA_VERSION for case in story2_cases)


def test_phase3_task6_story2_bundle_core_fields_are_stable(story2_cases):
...
def test_phase3_task6_story3_external_slice_is_bounded_and_exact():
...
def test_phase3_task6_story3_bundle_core_fields_are_stable():
...
def test_phase3_task6_story4_output_integrity_and_continuity_are_present(story4_cases):
...
def test_phase3_task6_story4_bundle_core_fields_are_stable(story4_cases):
...
def test_phase3_task6_story5_runtime_classifications_cover_full_matrix():
...
def test_phase3_task6_story5_bundle_core_fields_are_stable():
...
def test_phase3_task6_story6_negative_boundary_is_stage_separated():
    ...
    assert all(
        case["negative_record_schema_version"] == TASK6_NEGATIVE_RECORD_SCHEMA_VERSION
        for case in cases
    )


def test_phase3_task6_story6_bundle_core_fields_are_stable():
    bundle = build_story6_bundle(build_story6_cases())
    assert bundle["status"] == "pass"
    assert bundle["negative_record_schema_version"] == TASK6_NEGATIVE_RECORD_SCHEMA_VERSION
...
def test_phase3_task6_story7_correctness_package_is_complete():
    bundle = build_story7_bundle()
    assert bundle["status"] == "pass"
    assert bundle["schema_version"] == TASK6_CORRECTNESS_PACKAGE_SCHEMA_VERSION
    assert bundle["summary"]["total_cases"] == 25
...
def test_phase3_task6_story8_summary_consistency_closes_only_from_counted_supported_evidence():
    bundle = build_story8_bundle()
    assert bundle["status"] == "pass"
    assert bundle["schema_version"] == TASK6_SUMMARY_SCHEMA_VERSION
